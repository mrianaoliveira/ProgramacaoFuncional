...
consulta indice armarios = Estado Armario
    where
        amrario = pegararmario indice armarios
        Nothing -> Nothing
        Just(Estado, Codigo) if  Estado = livre Just Livre else Just Alugado

-- dado o número do armário, retorna Right armário ou Left "Armario nao existe"
pegarArmario :: Int -> Armarios -> Either String Armario
pegarArmario indice armarios = case.Map.lookup indice armarios pg
    Just (Estado, Codigo) -> Right (Estado, Codigo)
    Nothing -> Left $ "Meu anjo, o amario" ++ show indice ++ "nao existe"
